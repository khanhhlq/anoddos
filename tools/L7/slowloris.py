...
def create_socket(target):
    try:
        ...
        print(f"{Fore.RED}[-] {Fore.MAGENTA}Time out.{Fore.RESET}")
    except socket.error:
        print(f"{Fore.RED}[-] {Fore.MAGENTA}An error occurred while creating socket{Fore.RESET}")
    else:
    ...
    for _ in range(4):
        for index, sock in enumerate(sockets):
            try:
                ...
                print(
                    f"{Fore.RED}[-] {Fore.MAGENTA}An error occurred while sending keep-alive headers{Fore.RESET}"
                )
                sockets.remove(sock)
            else:
                print(
